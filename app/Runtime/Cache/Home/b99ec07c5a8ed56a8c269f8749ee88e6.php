...
<?php if(is_array($list)): foreach($list as $key=>$v): ?><article class="excerpt">
      ...
      <div class="focus">
        <a target="_blank" href="<?php echo bk_url('Index/deta',array('id'=>$v['id']));?>">
          <img style='height: 123px; width: 200px;'  class="thumb" src="<?php echo ($v['picurl'][0]['src']); ?>" alt="<?php echo ($v['title']); ?>"  data-original="<?php echo ($v['picurl'][0]['src']); ?>" /></a>
      </div>
...
$('.element').dropload({
    ...
    loadDownFn : function(me){
        $.ajax({
            type: 'GET',
            url: 'index.php?m=Index&a=catelist&page='+pageStart+'&ajax=1&id=<?php echo ($_GET['id']); ?>',
            dataType: 'html',
